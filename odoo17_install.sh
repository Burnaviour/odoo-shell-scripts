...
conda create -n odoo17-env python=3.9 -c conda-forge -y

# Activate the Conda Environment
echo -e "${GREEN}Activating Conda environment...${NC}"
conda activate odoo17-env
...
sudo mkdir -p /opt/odoo17_community/odoo
sudo mkdir -p /opt/odoo17_community/custom_addons

# Clone Odoo 17 Source
echo -e "${GREEN}Cloning Odoo 17 source from GitHub...${NC}"
sudo git clone https://github.com/odoo/odoo.git --depth 1 --branch 17.0 /opt/odoo17_community/odoo

# Create Odoo Configuration File
echo -e "${GREEN}Creating Odoo configuration file...${NC}"
sudo tee /opt/odoo17_community/odoo.conf << EOF
[options]
addons_path = /opt/odoo17_community/odoo/addons,/opt/odoo17_community/custom_addons
